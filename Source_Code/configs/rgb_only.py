# configs/rgb_only.py
# RGB-only detector configuration using QFDet backbone
...
# Model modifications for RGB only
model = dict(
    type='FasterRCNN',
    backbone=dict(
        type='ResNet',
    ...
    # For RGB only, we don't use thermal input
    train_cfg=dict(
        rpn=dict(
            assigner=dict(
                type='MaxIoUAssigner',
...
data = dict(
    ...
    train=dict(
        ...
        img_prefix='data/vtuav_subset/train/',
        pipeline=[
    ...
    val=dict(
        ...
        img_prefix='data/vtuav_subset/val/',
        pipeline=[
            dict(type='LoadImageFromFile'),
            dict(type='LoadAnnotations', with_bbox=True),
            dict(type='Resize', img_scale=(1333, 800), keep_ratio=True),
    ...
    test=dict(
        ...
        img_prefix='data/vtuav_subset/test/',
        pipeline=[
            dict(type='LoadImageFromFile'),
            dict(type='LoadAnnotations', with_bbox=True),
            dict(type='Resize', img_scale=(1333, 800), keep_ratio=True),
...
load_from = 'pretrained_models/qfdet_rgb_only.pth'